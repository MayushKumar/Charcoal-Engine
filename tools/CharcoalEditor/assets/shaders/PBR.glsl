		...
		if (NdotL >= 0)
			{
				...
				// Colour = vec4(D, D, D, 1.0f);
			}
	}
	Colour += vec4(albedo * 0.1, 1.0f);
	Colour = texture(cubeMap, transpose(fs_in.invTBNMatrix) * normal);
	// Colour = vec4(transpose(fs_in.invTBNMatrix) * vec3(0.0f, 0.0f, 1.0f), 1.0f);
	// Colour = vec4(texture(cubeMap, reflect(-viewDir, vec3(0.0f, 0.0f, 1.0f))).xyz, 1.0f);
}
