...
namespace Charcoal
{
	...
	void PerspectiveCameraController::OnUpdate(Timestep timestep)
	{
		if (m_Enabled)
		{
			if (Input::IsKeyPressed(CH_KEY_A))
				m_Camera.Translate(glm::vec3(-1.8f * timestep, 0.0f, 0.0f));
			if (Input::IsKeyPressed(CH_KEY_D))
				m_Camera.Translate(glm::vec3(1.8f * timestep, 0.0f, 0.0f));
			if (Input::IsKeyPressed(CH_KEY_W))
				m_Camera.Translate(glm::vec3(0.0f, 0.0f, -1.8f * timestep));
	...
	void PerspectiveCameraController::SetEnabled(bool enabled)
	{
		m_Enabled = enabled;
		if (m_Enabled)
		{
		...
		{
			Application::GetApplication().GetWindow().SetCursorMode(WindowCursorMode::Normal);
		}
	}
